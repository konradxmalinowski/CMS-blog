...
if (empty($email) || empty($password) || empty($name)) {
    http_response_code(400);
    echo json_encode(["success" => false, "message" => "Failed to get email or password or name from user"]);
    exit;
}
...
if (!$success) {
    ...
    echo json_encode(["success" => false, "message" => "Failed to check if user exists"]);
    exit;
}
